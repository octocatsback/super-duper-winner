...
Machine-readable pins live in [`packages.yaml`](packages.yaml).

## How projects were imported

| Method | When it is used |
| ------ | --------------- |
| Squash-equivalent subtree (vendored prefix) | Default for repos whose current tree fits in this umbrella |
...
| [`Gareth/`](Gareth/) | [octocatsback/Gareth](https://github.com/octocatsback/Gareth) | squash subtree (refreshed) | default branch `cursor/lock-down-environment-7a39` @ `60d4b85` |
...
`Gareth/`, `GitGoat/`, and `OriginTrials/` were already present from earlier work; they were refreshed in place rather than duplicated. `OriginTrials/` was already at the current `gh-pages` HEAD.
...
- **`Starlink/`**: Consumer dish grpcurl on `192.168.100.1:9200` is `get_status` / `reboot` / `dish_stow` / unstow. Router notes keep `get_diagnostics` for `Router-010000000000000001F29264` / SSID `Wanjer`. Public v2 stays stubbed until a Business service account exists. No cookies or passwords are stored.
